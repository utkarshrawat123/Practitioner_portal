...
describe('lessons', () => {
  it('inserts and reads a lesson round-tripping JSON fields', async () => {
    ...
    const id = await insertLesson(draft());
  ...
  it('lists by status newest first and edits then publishes', async () => {
    ...
    const a = await insertLesson(draft({ title: 'First' }));
    const b = await insertLesson(draft({ title: 'Second' }));
    expect((await listLessons()).map((l) => l.id)).toEqual([b, a]);
    expect(await listLessons('draft')).toHaveLength(2);
    await updateLessonFields(a, {
      title: 'Edited', summary: 'new', takeaways: ['x', 'y', 'z'],
      quiz: draft().quiz, topics: ['hormones'],
    });
    const published = await setLessonStatus(a, 'published');
    expect(published.title).toBe('Edited');
    expect(published.status).toBe('published');
    expect((await listLessons('published')).map((l) => l.id)).toEqual([a]);
  });
  ...
  it('filters published lessons by topic and search text', async () => {
    ...
    const a = await insertLesson(draft({ title: 'Iron basics', topics: ['iron-deficiency'], summary: 'ferritin low' }));
    const b = await insertLesson(draft({ title: 'Sleep science', topics: ['sleep'], summary: 'magnesium calm' }));
    await insertLesson(draft({ title: 'Still a draft', topics: ['sleep'] }));
    await setLessonStatus(a, 'published');
    await setLessonStatus(b, 'published');
    expect((await listPublishedLessons()).map((l) => l.id).sort()).toEqual([a, b].sort());
    expect((await listPublishedLessons({ topic: 'iron-deficiency' })).map((l) => l.id)).toEqual([a]);
    expect((await listPublishedLessons({ q: 'magnesium' })).map((l) => l.id)).toEqual([b]);
    expect((await listPublishedLessons({ q: 'iron' })).map((l) => l.id)).toEqual([a]);
  });
});
...
describe('completions', () => {
  it('toggles completion, counts, and refuses unpublished lessons', async () => {
    ...
    const pub = await insertLesson(draft());
    await setLessonStatus(pub, 'published');
    const stillDraft = await insertLesson(draft({ title: 'draft' }));

    expect(await toggleCompletion(p.id, pub)).toBe(true);
    expect(await countCompletions(p.id)).toBe(1);
    expect(await completedLessonIds(p.id)).toEqual([pub]);
    expect(await toggleCompletion(p.id, pub)).toBe(false);
    expect(await countCompletions(p.id)).toBe(0);

    expect(await toggleCompletion(p.id, stillDraft)).toBe(false);
    expect(await countCompletions(p.id)).toBe(0);
    expect(await toggleCompletion(p.id, 9999)).toBe(false);
  });
});
